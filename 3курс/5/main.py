...
def get_weather_week(lat, lon):
    ...
    for day in days_lst:
        weather[day["date"][5:10]] = day["parts"]["day"][
            "temp_avg"]  # записываем в словарь данные вида {дата: средняя температура}
...
def get_weather_hours(lat, lon):
    ...
    for day in days_lst:
        for hour in day['hours']:
            weather[str(day["date"][5:10]) + " " + str(hour['hour']) + "ч"] = hour[
                "temp"]  # формат данных {дата + ч: температура}
