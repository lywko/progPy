...
def get_currencies(currencies_ids_lst=[
    ...
    cur_res_str = requests.get(
        "http://www.cbr.ru/scripts/XML_daily.asp").text  # отправка запроса на апи и преобрахование ответа в текст
    result = dict()
...
def get_currencies_year(currencie_id='R01239'):  # функция получения валюты за год
    ...
    while date != today:  # итерируемся по циклу 365 раз, отправляя каждый раз запрос
        ...
        if len(str(
                date.day)) == 1:  # так как по запросу ожидается формат дд:мм:гггг то стоит проверять является ли число однозначным
        ...
        req = requests.get('http://www.cbr.ru/scripts/XML_daily.asp',
                           params={'date_req': day + '/' + month + '/' + str(date.year)}).text  # отправка запроса
